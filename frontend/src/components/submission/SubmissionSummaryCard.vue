  ...
  <section class="summary-card" :class="[`summary-card--${statusTone}`, { 'summary-card--embedded': embedded }]"
    ...
    <dl class="facts">
      <div class="fact">
        <dt>提交者</dt>
        <dd>
          <button class="user-link" type="button" :disabled="!user.id" @click="emit('userClick', user.id)">
      ...
      <div class="fact">
        <dt>题目</dt>
        <dd>
          <button class="problem-link" type="button" @click="emit('problemClick', problem.id)">
            {{ problem.name || "—" }}
          </button>
        </dd>
      </div>
      <div class="fact">
        <dt>语言</dt>
        <dd class="mono">{{ submission.language || "—" }}</dd>
      </div>
      <div class="fact">
        <dt>用时</dt>
        <dd class="mono">{{ timeText }}</dd>
      </div>
      <div class="fact">
        <dt>内存</dt>
        <dd class="mono">{{ memoryText }}</dd>
      </div>
      <div class="fact">
        <dt>通过</dt>
        <dd class="mono">{{ passText }}</dd>
      </div>
    </dl>
  </section>
</template>
